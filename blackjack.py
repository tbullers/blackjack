...
def intro_text():
    ...
    print(f"You start with a bankroll of ${start_bank_role}.  Your least bet is ${min_bet}.\n\n")
    print('Good luck!\n\n')
...
class CreateAccount:
    def __init__(self, start_balance):
        ...
        if valid:
            self.balance = amount
            return (True, 'The account was created and the current balance is: $ '+self.balance)
        else:
    ...
    def withdraw(amount):
        ...
        if valid:
    	    self.balance -= amount
            return (True, 'The amount was withdrawn and the new balance is: $'+self.balance)
    	else:
           return (False, 'There was an error and the withdrawal did not proceed.  The balance is: $'+self.balance)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        print('Your account balance is $ '+player_account.balance()+'\n')
        response = str(input('How much would you like to bet? (must be between $ '+min_bet+' and $ '+player_account.balance()))
